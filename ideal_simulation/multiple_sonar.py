...
def plot_both_views(world, y_range, z_range, sonar_positions, all_sonar_data, angles, angle_width, max_range, all_theta, plot=True):
    ...
    min_angle = np.min([np.min(t) for t in all_theta]) if all_theta else 0
    max_angle = np.max([np.max(t) for t in all_theta]) if all_theta else 0
    ref_angle = 0  # Default reference angle
